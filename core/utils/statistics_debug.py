...
def draw_equity_and_send_pic(me_conf, equity_df, transfer_df, title, account_config: AccountConfig, is_send=True):
    ...
    # 确保equity_df的时间列也不带时区，保持一致性
    equity_df['time'] = equity_df['time'].dt.tz_localize(None)
    equity_df['time']  = equity_df['time'] + time_diff
    ...
    if not is_send:
        ...
        for conf in me_conf.factory.config_list:
            sub_stg_equity = pd.read_csv(conf.get_result_folder() / '资金曲线.csv', parse_dates=['candle_begin_time'], index_col=0)
    ...
    # =画图
    fig, (ax1, ax3, ax4, ax5, ax6) = plt.subplots(5, 1, figsize=(12, 14), gridspec_kw={'height_ratios': [3, 1, 1, 1, 3]})
    # 标记买入和卖出点
    ...
    for i in range(len(equity_df) - 1):
        ...
        ax1.plot([start_time, end_time], [start_net, end_net], color=color, zorder=3, linestyle='--')
    ...
    for idx, conf in enumerate(me_conf.factory.config_list):
        ...
        # 确保时间比较的一致性：将equity_df的时间也转换为不带时区的格式
        comparison_time = (equity_df['time'].min() - time_diff).tz_localize(None) if equity_df['time'].min().tz is not None else equity_df['time'].min() - time_diff
        sub_stg_equity = sub_stg_equity[sub_stg_equity['candle_begin_time'] >= comparison_time].reset_index(drop=True)
    ...
    # 增加资金曲线副图
    ax6.scatter(buy_signals['time'], buy_signals['net'], marker='+', color='black', label='add', s=100)
    ax6.scatter(sell_signals['time'], sell_signals['net'], marker='x', color='red', label='reduce', s=100)
    ax6.plot(equity_df['time'].to_numpy() , equity_df['net'].to_numpy(), color=equity_df['color'].iloc[-1], zorder=3)
    set_annotate(ax6, equity_df['color'].iloc[-1], equity_df['time'].to_numpy() , equity_df['net'].to_numpy())
    ...
    ax6.grid(True, which='both', linestyle='--', linewidth=0.5, zorder=1)
    ax6.set_title(f'{title} dd2here: {equity_df.iloc[-1]["dd2here"]:.2f}% eq_max: {equity_df["net"].max():.2f}%')
    ax6.yaxis.set_major_formatter(yticks)

    # 创建右侧轴
    # 右轴 回撤
    ax7 = ax6.twinx()
    ax7.fill_between(equity_df['time'], equity_df['dd2here'], 0, color='#d5d5d5', alpha=0.2, zorder=0)
    ax7.set_ylabel('dd2here (%)')
...
def save_and_send_equity_info(account_config: AccountConfig, me_conf, spot_position, swap_position, spot_equity, account_equity, spot_usdt):
    ...
    """
    # =新建文件夹路径，保存数据
    equity_file_path = get_file_path(data_path, '仓位管理回测结果', account_config.name, '资金曲线.csv')
    # =判断文件是否存在
    if os.path.exists(equity_file_path):  # 如果存在
        # 读取数据
        equity_df = pd.read_csv(equity_file_path, parse_dates=['candle_begin_time'])
        equity_df.rename(columns={'candle_begin_time': 'time',
                                  'equity': '账户总净值', 'long_pos_value': '多头仓位', 'short_pos_value': '空头仓位',
                                  'symbol_long_num': '多头选币', 'symbol_short_num': '空头选币'}, inplace=True)  # 修改时间列名
        equity_df['多头单币最大持仓'] = 0
        equity_df['空头单币最大持仓'] = 0
        # 容错代码，转换时间周期
        ...
        draw_equity_and_send_pic(me_conf, equity_df0, transfer_df, 'Simulate equity-curve(All days)', account_config, is_send=False)
        # =构建近30天数据
        duration = '720h'
        equity_df1 = equity_df_temp[equity_df_temp['time'] >= equity_df_temp['time'].max() - pd.to_timedelta(duration)].reset_index(drop=True)
        # 绘图
        draw_equity_and_send_pic(me_conf, equity_df1, transfer_df, 'Simulate equity-curve(last 30 days)', account_config)

        # =构建近7天数据
        duration = '168h'
        equity_df2 = equity_df_temp[equity_df_temp['time'] >= equity_df_temp['time'].max() - pd.to_timedelta(duration)].reset_index(drop=True)
        # 绘图
        draw_equity_and_send_pic(me_conf, equity_df2, transfer_df, 'Simulate equity-curve(last 7 days)', account_config)
    return 'equity_msg'
...
def send_pos_strategy_info(me_conf: MultiEquityBacktestConfig, account_info: AccountConfig):
    ...
    if me_conf.strategy.name in ['RotationStrategy', 'RotationStrategyOffset', 'MultiRotationStrategy']:
    ...
    for account_profile_file in load_multi_accounts():
        ...
        if run_time is None:
            ...
            if not (datetime.now() - pd.to_timedelta('5m') <= run_time <= datetime.now() + pd.to_timedelta('5m')):
                ...
                continue
        # 以下代码可以测试的时候使用
        # run_time = datetime.strptime('2023-10-31 11:20:00', "%Y-%m-%d %H:%M:%S")
        ...
        # ===生成账户净值信息
        equity_msg = save_and_send_equity_info(account_info, me_conf, None, None, None, None, None)
        print(f'账户【{account_name}】净值信息已发送: {equity_msg}')
